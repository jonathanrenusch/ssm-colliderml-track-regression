...
from track_regression.seed import (DEFAULT_BZ, KAPPA, PIXEL_VOLUMES, compress_residuals, estimate_free,
                                   ...
                                   seed_perigee, seed_residuals, select_triplet)
...
def test_torch_seed_and_residuals_match_numpy():
    torch = pytest.importorskip("torch")
    from track_regression.seed_torch import compress_residuals_torch, seed_perigee_torch, seed_residuals_torch
    n = 300
    d0, z0, phi, theta, qop = random_params(n)
    ...
    r_t = compress_residuals_torch(seed_residuals_torch(
        torch.from_numpy(flat), torch.from_numpy(s_np), torch.from_numpy(track))).numpy()
