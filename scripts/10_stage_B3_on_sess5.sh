#!/bin/bash
# Stage the 3 T-target stores on a machine's /scratch (from /eos), build the eval farm and the
...
EOS=/eos/project/e/end-to-end-colliderml/data/ICLR_retraining_geom_B3
OUT=/scratch/colliderml/ICLR_retraining_geom_B3; EV=/scratch/colliderml/ICLR_eval_geom_B3
...
pixi run -e default python $REPO/scripts/07_build_mixed_store.py --base $OUT/single_muon_uniform --extra $OUT/ttbar_new_pt1_tr \
   --extra-max-tracks $MAXX --extra-val --out /scratch/colliderml/ICLR_retraining_geom_B3_mixed16M
echo "STAGED $(date)"
