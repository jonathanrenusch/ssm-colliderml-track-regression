...
#   eval_root  default /scratch/colliderml/ICLR_eval_geom (geometry-ordered stores);
#              ICLR_eval_ssort = s-ordered twin; ICLR_eval = deprecated time-sorted
...
set -euo pipefail
RUN_DIR="$(realpath "$1")"; CKPT_NAME="$2"; OUT="$(realpath -m "$3")"; EVAL_ROOT="${4:-/scratch/colliderml/ICLR_eval_geom}"; GPU="${5:-0}"
...
for ds in single_muon_2GeV single_muon_10GeV single_muon_100GeV ttbar ttbar_new_pt1 single_muon_uniform; do
  [ -d "$EVAL_ROOT/$ds/test" ] || { echo "=== $ds: not in $EVAL_ROOT, skipped"; continue; }   # ttbar_new_pt1 (§4.7) exists on the geom farm only
  echo "=== $ds  $(date)"
