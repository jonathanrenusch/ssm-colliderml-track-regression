#!/bin/bash
# Launch trials A and C on GPUs 0-1 of the OTHER machine, one process per GPU, via nohup.
# Run this ON that machine. Stop the old 10 L run (baeedc59, time-scrambled store) first —
# the busy-GPU check below refuses to start on a GPU that still has a compute process.
set -uo pipefail
...
launch A_4L_ds64_lion_cosine_bs2048   0
launch C_10L_ds128_lion_cosine_bs2048 1
