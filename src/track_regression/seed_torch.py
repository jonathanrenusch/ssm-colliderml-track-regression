...
offsets.  The maths runs in float64 and the results are cast to the input
dtype: the perigee transport and the per-hit residuals both difference against
the helix radius R (rc - R, rho - R), and at 100 GeV R is ~1.1e5 mm, so fp32
would inject O(R * eps) ~ 10 um of noise.  Parity with the numpy path is tested
in tests/test_seed_torch.py.
"""
...
def gpu_seed_features(hit_features: torch.Tensor, cu_seqlens: torch.Tensor, bz: float = DEFAULT_BZ,
                      max_len: int | None = None, dtype: torch.dtype = torch.float64):
    """Seed (B, 5) and compressed residual features (n_hits, 3) on the device of the inputs.

    ``max_len`` (the store's max hits per track, 20) avoids a host sync for the
    padded length.  ``dtype`` is the internal maths precision."""
    dt = dtype
